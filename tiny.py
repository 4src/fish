...
def SYM(at=0,name=""):
  return BOX(this=SYM, name=name, at=at, n=0, has={}, most=0, mode=None, filter=lambda x:x)

def NUM(at=0,name=""):
  return BOX(this=NUM, name=name, at=at, n=0, mu=0, m2=0, hi= -1E30, lo=1E30, 
             filter=int,
...
def key(rule, new):
  if new.this is RULE:
    [key(rule,k) for lst in new.cols.values() for k in lst]
  else:
...
def selects(rule,row):
  ...
  for keys in rule.cols.values():
    if not select1(keys): return False
  return True
...
def keys(best,rest):
  ...
  def num():
    ...
    f    = best.filter
    return set(( (f(lo), f(hi)) for lo,hi in tmp ))
  #----------------------------------------
  for lo,hi in ( sym() if best.this is SYM else num() ):
